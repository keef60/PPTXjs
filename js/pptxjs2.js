...
(function (global) {
    function PPTXJS(options) {
        console.log(1325435345)
        ...
        var settings = Object.assign({
            ...
            slideModeConfig: {
                ...
                autoSlide: true, /* false or seconds, F8 to activate, keyBoardShortCut: true */
                randomAutoSlide: false, /* true, false, autoSlide: true */
                loop: false, /* true, false */
                background: false, /* false or color */
                transition: "default", /* transition type: "slide", "fade", "default", "random" */
        ...
        loadingMsgDiv.className = "slides-loading-msg";
        loadingMsgDiv.style = "display:block; width:100%; color:white; background-color: #ddd;";
        var progressBarDiv = document.createElement("div");
        progressBarDiv.className = "slides-loading-progress-bar";
        progressBarDiv.style = "width: 1%; background-color: #4775d1;";
        progressBarDiv.innerHTML = "<span style='text-align: center;'>Loading... (1%)</span>";
        loadingMsgDiv.appendChild(progressBarDiv);
        result.prepend(loadingMsgDiv);
        ...
        function convertToHtml(file) {
            ...
            if (file.byteLength < 10){
                console.error("file url error (" + settings.pptxFileUrl + "0)");
                document.querySelector(".slides-loadnig-msg").remove();
                return;
            }
            var zip = new JSZip(), s;
            ...
            //var slidesHeight = document.getElementById(divId).querySelector(".slide").offsetHeight;
            for (var i = 0; i < rslt_ary.length; i++) {
                switch (rslt_ary[i]["type"]) {
                    case "slide":
                        document.getElementById(divId).insertAdjacentHTML('beforeend', rslt_ary[i]["data"]);
                        break;
                    case "pptx-thumb":
                        //document.getElementById("pptx-thumb").setAttribute("src", "data:image/jpeg;base64," +rslt_ary[i]["data"]);
                        break;
                    case "slideSize":
                        slideWidth = rslt_ary[i]["data"].width;
                        slideHeight = rslt_ary[i]["data"].height;
                        /*
                        document.getElementById(divId).style.width = (slideWidth + 80) + "px";
                        document.getElementById(divId).style.height = (slideHeight + 60) + "px";
                        */
                        break;
                    case "globalCSS":
                        //console.log(rslt_ary[i]["data"])
                        document.getElementById(divId).insertAdjacentHTML('beforeend', "<style>" + rslt_ary[i]["data"] + "</style>");
                        break;
                    case "ExecutionTime":
            ...
            if (!settings.slideMode || (settings.slideMode && settings.slideType == "revealjs")) {
                if (document.getElementById("all_slides_warpper") === null) {
                    var slides = document.getElementById(divId).querySelectorAll(".slide");
                    var allSlidesWrapper = document.createElement("div");
                    allSlidesWrapper.id = "all_slides_warpper";
                    allSlidesWrapper.className = "slides";
                    slides.forEach(slide => allSlidesWrapper.appendChild(slide));
                    document.getElementById(divId).appendChild(allSlidesWrapper);
                }
            ...
            var slidesHeight = document.getElementById(divId).querySelector(".slide").offsetHeight;
            var numOfSlides = document.getElementById(divId).querySelectorAll(".slide").length;
            var sScaleVal = (sScale !== "") ? scaleVal : 1;
            //console.log("slidesHeight: " + slidesHeight + "\nnumOfSlides: " + numOfSlides + "\nScale: " + sScaleVal)
        
            document.getElementById("all_slides_warpper").style.cssText = trnsfrmScl + ";height: " + (numOfSlides * slidesHeight * sScaleVal) + "px";
            //}
        }
        ...
        function initSlideMode(divId, settings) {
            ...
            if (settings.slideType == "" || settings.slideType == "divs2slidesjs") {
                ...
                var slidesHeight = slideElements.length > 0 ? slideElements[0].offsetHeight : 0;
                ...
                setTimeout(function () {
                    ...
                    document.querySelectorAll(".slides-loadnig-msg").forEach(function(msg) {
                        msg.remove();
                    });
        
                    // Assuming divs2slides is a custom function that needs to be replaced or implemented in vanilla JS
                    divs2slides(divId, {
            ...
            for (var i = 0; i < apNode.length; i++) {
                ...
                if (rNode === undefined && pNode !== undefined) {
                    ...
                    if (isBullate) {
                        ...
                        txt_obj.style.position = 'absolute';
                        txt_obj.style.float = 'left';
                        txt_obj.style.whiteSpace = 'nowrap';
                        txt_obj.style.visibility = 'hidden';
                        txt_obj.innerHTML = prgr_text;
                        document.body.appendChild(txt_obj);
                        total_text_len += txt_obj.offsetWidth;
                        document.body.removeChild(txt_obj);
                    }
    ...
    */
    ...
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = PPTXJS;
    } else {
        global.PPTXJS = PPTXJS;
    }
